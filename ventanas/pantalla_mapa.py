"""
ventanas/pantalla_mapa.py
-------------------------
Pantalla de mapa del juego. Cumple el requerimiento 01:

  - Muestra 5 ubicaciones (tierras) con sus Hollows.
  - El jugador solo puede moverse entre las 5 ubicaciones.
  - Indica el progreso del jugador (cuántos Hollows derrotó).
  - Botón para PELEAR contra el Hollow de la ubicación actual.

Diseño:
  Las 5 ubicaciones se muestran como botones en fila.
  La ubicación activa se resalta en dorado.
  Un panel inferior muestra el estado del jugador.

Conceptos para la defensa:
  - Estado del juego: guardamos en variables de Python qué
    ubicaciones ya fueron completadas (lista de booleanos).
  - lambda en Tkinter: los botones de ubicación usan lambda
    para pasar el índice correcto al comando, porque Python
    captura variables por referencia en bucles.
"""
...
def crear_pantalla_mapa(
    ...
    def ir_a_batalla():
        ...
        El Hollow recibe 3 personajes aleatorios de la lista completa.
        """
        idx = seleccion[0]
        if completadas[idx]:
